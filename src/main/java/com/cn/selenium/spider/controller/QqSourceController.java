...
import com.cn.selenium.spider.service.IQqSourceService;
...
public class QqSourceController {
	@Resource
	IQqSourceService sourceService;
	...
	public Result list(QqSource  qqSource,
		...
		try {
			return Result.SUCCESS(sourceService.page(new Page<>(pageNum, pageSize), new QueryWrapper<>(qqSource)));
		} catch (Exception e) {
	...
	public Result delete(@PathVariable(name = "id") Integer id) {
		try {
			boolean remove = sourceService.removeById(id);
	 ...
	 */
	@PostMapping("/update")
	public Result update(@RequestBody QqSource qqSource) {
		try {
			sourceService.updateById(qqSource);
